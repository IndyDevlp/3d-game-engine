...
namespace EngineCore {
	...
	void Camera::update_view_matrix()
	{
		...
		const float pich_in_rad	= glm::radians(-m_rotation.y);
		const float yaw_in_rad	= glm::radians(-m_rotation.z);
		...
		const glm::mat3 rotate_matrix_y(cos(pich_in_rad),	0, -sin(pich_in_rad), 
										0,					1,  0,					
										sin(pich_in_rad),	0,  cos(pich_in_rad));

		const glm::mat3 rotate_matrix_z(cos(yaw_in_rad),	sin(yaw_in_rad), 0, 
